...
export function WriterPage() {
  ...
  const KanbanColumn = ({ title, status, count, color }: { title: string, status: string, count: number, color: string }) => {
    const columnOrders = myOrders.filter(o => {
        if (status === "New") return o.status === "Pending" || o.status === "In Progress"; 
        // Mapping generic statuses for demo purposes since we don't have all statuses in the mock
        if (status === "Drafting") return o.status === "In Progress";
        if (status === "Review") return o.status === "Under Review"; // Mock status
        if (status === "Completed") return o.status === "Completed";
        return o.status === status;
    });

    // Mock distribution for visual effect if statuses don't match exactly
    const displayOrders = title === "New Orders" ? myOrders.filter(o => o.status === "Pending") :
                          title === "Drafting" ? myOrders.filter(o => o.status === "In Progress") :
                          title === "Client Review" ? [] :
                          title === "Completed" ? myOrders.filter(o => o.status === "Completed") : [];

    return (
      <div className="flex-1 min-w-[280px] bg-secondary/30 rounded-xl p-4 flex flex-col h-full max-h-[calc(100vh-200px)]">
        <div className={`flex items-center justify-between mb-4 pb-2 border-b border-${color}-200`}>
          ...
          <Badge variant="secondary" className="bg-background">{displayOrders.length}</Badge>
        </div>
           ...
           <div className="space-y-3">
             {displayOrders.map(order => (
               <Card 
                 ...
                 onClick={() => { setSelectedOrderId(order.id); setViewMode("list"); }}
               >
             ...
             {displayOrders.length === 0 && (
                <div className="h-24 border-2 border-dashed rounded-lg flex items-center justify-center text-muted-foreground text-sm italic opacity-50">
                  No orders
                </div>
             )}
           </div>
      ...
      <header className="border-b bg-card px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-secondary p-1 rounded-lg">
            ...
            >
              <ListIcon className="w-3 h-3 mr-2" /> List
            </Button>
            <Button 
      ...
      {viewMode === "board" ? (
        <main className="flex-1 p-6 overflow-hidden">
           <div className="flex h-full gap-6 overflow-x-auto pb-4">
             <KanbanColumn title="New Orders" status="Pending" count={2} color="blue" />
             <KanbanColumn title="Drafting" status="In Progress" count={1} color="yellow" />
             <KanbanColumn title="Client Review" status="Under Review" count={0} color="purple" />
             <KanbanColumn title="Completed" status="Completed" count={12} color="green" />
           </div>
        </main>
      ) : (
